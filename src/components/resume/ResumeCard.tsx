...
const ResumeCard = ({ title, desc, result, subTitle }: any) => {
  console.log(title, desc, result, subTitle);

  return (
    <div className="w-full h-1/3 group flex">
      <div className="w-10 h-[6px] bg-opacity-40 bg-black mt-16 relative">
        <span className="absolute w-5 h-5 rounded-full -top-2 -left-3 flex justify-center items-center bg-black bg-opacity-60">
          <span className="w-3 h-3 rounded-full bg-bodyColor inline-flex group-hover:bg-designColor duration-300 "></span>
        </span>
      </div>
      <div className="w-full bg-black bg-opacity-20 hover:bg-opacity-30 duration-300 rounded-lg pz-10 flex flex-col justify-center gap-10 shadow-shadowOne p-4">
